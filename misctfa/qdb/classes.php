...
if (defined('INSTALLED')) {
    ...
    $t = mysql_result($db->_sql($sql),0);
    $sql = "SELECT COUNT(id) FROM ".$_qdbs['tpfx']."queue";
    $p = mysql_result($db->_sql($sql),0);
    $tpl->set('t', $t);
    $tpl->set('p', $p);
    $tpl->set('version', '1.11');
    if(!isset($_SESSION['loggedin']) && isset($_COOKIE['qdb_username']) && isset($_COOKIE['qdb_password'])) {
        $sql = "SELECT * FROM ".$_qdbs['tpfx']."admins WHERE username='".mysql_real_escape_string(strtolower($_COOKIE['qdb_username']))."' LIMIT 1";
        $r = $db->_sql($sql);
        $row = $db->fetch_row($r);
        if (($row['password'] == $_COOKIE['qdb_password']) and ($row['username'] == strtolower($_COOKIE['qdb_username']))) {
            $sql = "UPDATE ".$_qdbs['tpfx']."admins SET ip='$ip' WHERE username='".mysql_real_escape_string($_COOKIE['qdb_username'])."' LIMIT 1";
            $r = $db->_sql($sql);
