...
if (!empty($_GET['do']) || !empty($_POST['do'])) {
    ...
    if ((!empty($_POST['q']) || !empty($_GET['q'])) && ((!empty($_GET['p']) && ($_GET['p'] == 'search')) || (!empty($_GET['do']) && ($_GET['do'] == 'search'))) ) {
        ...
        if ( !empty($_POST['q']) ) {
	        $sql = "SELECT * FROM ".$_qdbs['tpfx']."quotes WHERE quote LIKE '%".mysql_real_escape_string($_POST['q'])."%' LIMIT ".intval($start).", ".intval($pgr->limit);
        } else {
	        $sql = "SELECT * FROM ".$_qdbs['tpfx']."quotes WHERE quote LIKE '%".mysql_real_escape_string($_GET['q'])."%' LIMIT ".intval($start).", ".intval($pgr->limit);
        }
        $r = $db->_sql($sql);
	...
	if (!empty($matches[1])) {
        ...
        $sql = "SELECT * FROM ".$_qdbs['tpfx']."quotes WHERE id='".mysql_real_escape_string($id)."' LIMIT 1";
        $r = $db->_sql($sql);
        if (mysql_num_rows($r) > 0) {
            $row = $db->fetch_row($r);
            $tpl->set('q_id', $row['id']);
            $tpl->set('q_rating', $row['rating']);
            $tpl->set('quote', $row['quote']);
            $sql = "SELECT ip FROM ".$_qdbs['tpfx']."votes WHERE id='".$row['id']."' AND ip='".mysql_real_escape_string($ip)."'";
            $r2 = $db->_sql($sql);
            $row2 = $db->fetch_row($r2);
            if ($row2['ip'] != $ip) {
                $rate = $tpl->fetch($tpl->tdir.'quote_rate.tpl');
