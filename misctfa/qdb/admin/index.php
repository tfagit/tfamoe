    ...
    if (!empty($_SESSION['loggedin'])) {
        switch ($_GET['do']) {
            ...
            case 'remove':
                 ...
                 $sql = "SELECT * FROM ".$_qdbs['tpfx']."admins WHERE username='".mysql_real_escape_string($_COOKIE['qdb_username'])."' LIMIT 1";
                 $r = $db->_sql($sql);
                 $row = $db->fetch_row($r);
                 if ($row['level'] == '2') {
                     $sql = "DELETE FROM ".$_qdbs['tpfx']."admins WHERE id='".mysql_real_escape_string($_GET['id'])."'";
                     $r = $db->_sql($sql);
                 }
            ...
            case 'raise':
                 ...
                 $row = $db->fetch_row($r);
                 if ($row['level'] == '2') {
                     $sql = "SELECT * FROM ".$_qdbs['tpfx']."admins WHERE id='".mysql_real_escape_string($_GET['id'])."' LIMIT 1";
                     $r = $db->_sql($sql);
                     $row = $db->fetch_row($r);
                     if ($row['level'] < '2') {
                         $sql = "UPDATE ".$_qdbs['tpfx']."admins SET level=level+1 WHERE id='".mysql_real_escape_string($_GET['id'])."' LIMIT 1";
                         $r = $db->_sql($sql);
                     }
                 }
        ...
        switch ($_POST['do']) {
            case 'add':
                ...
                if ($row['level'] == '2') {
                    ...
                    $sql = "INSERT INTO ".$_qdbs['tpfx']."admins (username,password,ip,id) VALUES ('".mysql_real_escape_string($username)."', '".mysql_real_escape_string($password)."', 'NULL', 'NULL')";
                    $r = $db->_sql($sql);
                    $tpl->set('logged', $tpl->fetch('.'.$tpl->tdir.'admin_links.tpl'));
                    print($tpl->fetch('.'.$tpl->tdir.'admin_header.tpl'));
            ...
            case 'change':
                ...
                if (($c_password == $row['password']) && ($c_password1 == $c_password2)) {
                    $sql = "UPDATE ".$_qdbs['tpfx']."admins SET password='".mysql_real_escape_string($c_password1)."' WHERE username='".mysql_real_escape_string($_COOKIE['qdb_username'])."' LIMIT 1";
                    $r = $db->_sql($sql);
                    $tpl->set('logged', $tpl->fetch('.'.$tpl->tdir.'admin_links.tpl'));
                    print($tpl->fetch('.'.$tpl->tdir.'admin_header.tpl'));
                    print($tpl->fetch('.'.$tpl->tdir.'admin_success.tpl'));
    ...
    if (!empty($_POST['do'])) {
        switch ($_POST['do']) {
            case 'login':
                ...
                $sql = "SELECT * FROM ".$_qdbs['tpfx']."admins WHERE username='".mysql_real_escape_string($username)."' LIMIT 1";
                $r = $db->_sql($sql);
                $row = $db->fetch_row($r);
                if (strtolower($row['password']) == $password) {
                    $sql = "UPDATE ".$_qdbs['tpfx']."admins SET ip='".mysql_real_escape_string($ip)."' WHERE username='".mysql_real_escape_string($username)."' LIMIT 1";
                    $r = $db->_sql($sql);
